...
$(document).ready(function () {
  ...
  cropestWDC_table = $("#crestWDC_table_report").DataTable({
    select: {
      style: "multiple",
      blurable: true,
    },
    ...
    ajax: {
      method: "GET",
      url: "get/weekly_data_collectorCREST",
      dataSrc: "",
    },
    columns: [
      { data: "registration_number" },
        ...
        if (Object.hasOwnProperty.call(respuesta, key)) {
          ...
          $("#tb_cropest").append(
            "<tr>" +
              ...
              "</td>" +
              "<td>" +
              element["date_planted"] +
              "</td>" +
              "<td>" +
              element["variety"] +
              "</td>" +
              "<td>" +
              element["stage_maturity"] +
              "</td>" +
              "<td>" +
              ...
              "</td>" +
              "<td>" +
              element["activities_carried"] +
              "</td>" +
              "<td>" +
              element["taq_arf_moa"] +
              "</td>" +
              "<td>" +
              element["n_farm_visits"] +
              "</td>" +
              "<td>" +
              element["remarks"] +
              "</td>" +
...
function edit_row_cropest(data) {
  ...
  $("#CE_county_list_id option").each(function () {
    if ($(this).val() == data["id_lv1"]) {
      $(this).attr("selected", "selected").trigger("change");
    }
  });
  $("#CE_sub_county_list_id option").each(function () {
    if ($(this).val() == data["id_lv2"]) {
      $(this).attr("selected", "selected").trigger("change");
    }
  });
  $("#CE_list_comp option").each(function () {
    if ($(this).attr("id") == data["id_lv3"]) {
      $("#CE_village_id").val($(this).val());
      $(this).attr("selected", "selected").trigger("change");
    }
  });
...
function delete_row_cropest(data) {
  $.ajax({
    url: "delete/weekly_data_collector",
    type: "POST",
    data: {
      id_praedial: data["id_praedial"],
    },
    headers: { "X-Requested-With": "XMLHttpRequest" },
    dataType: "json",
    success: function (respuesta) {
