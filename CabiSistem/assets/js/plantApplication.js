...
$("#county_list_PA").on("change", function () {
  $("#sub_county_list_PA").empty();
  $("#sub_county_list_PA").append(
...
$("#sub_county_list_PA").on("change", function () {
  $("#list_comp_PA").empty();
  $("#village_PA").val("");
    ...
    columnDefs: [
      {
        ...
        render: function (data, type, row) {
          return (
            ...
            " " +
            row.mo_last_name +
            " </span><input class='id_dca_form' type='hidden' name='id_dca_form' value=" +
            row.id_dca_form +
            ">"
          );
        },
...
$("#btn_scomplPA").on("click", function () {
  ...
  if (r) {
    $.ajax({
      ...
      type: "POST",
      data: {
        name_lv3: $("#village_PA").val(),
        id_lv2: $("#sub_county_list_PA").val(),
      },
      headers: { "X-Requested-With": "XMLHttpRequest" },
      dataType: "json",
        ...
        if (Object.hasOwnProperty.call(respuesta, key)) {
          ...
          $("#papp_tb1").append(
            "<tr>" +
              ...
              "</td>" +
              "<td>" +
              element["plant_approv"] +
              "</td>" +
              "<td>" +
              element["plant_received"] +
              "</td>" +
              "</tr>"
          );
        }
      ...
      for (const key in respuesta) {
        if (Object.hasOwnProperty.call(respuesta, key)) {
          ...
          $("#papp_tb2").append(
            "<tr>" +
              "<td>" +
              element["pre_date_visit"] +
              "</td>" +
              "<td>" +
              element["pre_com"] +
              "</td>" +
              "<td>" +
              element["extn_officeer"] +
              "</td>" +
              "<td>" +
              element["supervisor"] +
              "</td>" +
      ...
      for (const key in respuesta) {
        if (Object.hasOwnProperty.call(respuesta, key)) {
          ...
          $("#papp_tb3").append(
            "<tr>" +
              "<td>" +
              element["pos_date_visit"] +
              "</td>" +
              "<td>" +
              element["pos_Comments"] +
              "</td>" +
              "<td>" +
              element["extn_officeer_post"] +
              "</td>" +
              "<td>" +
              element["supervisor_post"] +
              "</td>" +
              "</tr>"
          );
        }
...
function to_pdf_pappform(papp) {
  ...
  doc.text(30, 155, "Parish: " + papp["name_lv1"]);
  doc.text(180, 155, "Village: " + papp["name_lv2"]);
  doc.text(350, 155, "Compment adress: " + papp["name_lv3"]);
  ...
  doc.text(
    ...
    "‘Quantity Recommended’ to be done by Extension Officer; ‘Quantity Approved’ to be done by Agronomy Division "
  );
  doc.text(
    90,
    doc.autoTableEndPosY() + 30,
    "NB: Forms to be submitted to the Extension Officer to reach Agronomy Division no later than September 15"
  );
  doc.setFontType("normal");
  doc.text(
    150,
    doc.autoTableEndPosY() + 40,
    " (Please note the date the form has been received from the applicant) "
  );
  ...
  doc.text(
    30,
    50,
    "1. Pre-planting inspection: (eg. Area cleared, status of lining of plots, drains, establishment of shade, holes dug, etc)"
  );
  ...
  doc.text(
    ...
    "2. Post-planting inspection: (eg. Plants planted; field condition – weeds, rootstock growth;  pests & diseases ; # of deaths; cause  of death, etc.)"
  );
  ...
  doc.text(
    30,
    doc.autoTableEndPosY() + 20,
    "3. Potential for Future Development: (eg. Plans for expansion, overall plan for farm, etc.)"
  );
  ...
  doc.text(
    ...
    "Extension Officer: ______________________________	Date:________________________"
  );
...
function edit_row_plantApplications(data) {
  ...
  $(".plant_application input[name='id_plant_apply']").val(
    data["id_plant_apply"]
  );
...
function delete_row_papp(data) {
  $.ajax({
    url: "delete/plant_application",
    type: "POST",
    data: {
      id_plant_apply: data["id_plant_apply"],
    },
    headers: { "X-Requested-With": "XMLHttpRequest" },
    dataType: "json",
    success: function (respuesta) {
