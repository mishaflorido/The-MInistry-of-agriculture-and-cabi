...
function to_pdf_dcaform(dca) {
  ...
  doc.text(30, 10, "Grenada and Ministry of agricultural, lands and forestry");
  doc.setFontSize(10);
  // clinic information
  ...
  doc.text(
    120,
    35,
    "Plant Doctor: " + dca["pdoc_name"] + " " + dca["pdoc_lastname"]
  );
  // Farmer information
  ...
  doc.text(
    14,
    155,
    "(Adtional information - Include key observed symptoms, plan part afected, etc  "
  );
  ...
  //Aca falta hacer la seleccion para poner la x
  if (dca["type_problem"] != null) {
    console.log(dca["type_problem"]);
  ...
  doc.text(
    14,
    245,
    "Recommendation to prevent this problem for the current problem: "
  );
...
function edit_row_dcaform(data) {
  ...
  $("input[name='id_plant_doc']").val(
    data["pdoc_name"] + " " + data["pdoc_lastname"]
  );
  ...
  $("#list_comp option").each(function () {
